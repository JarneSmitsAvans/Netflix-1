...
public class WatchBehaviourDAO {
    ...
    public ArrayList<Movie> getWatchedMovies(int profileID) throws SQLException, ClassNotFoundException {
        ...
        while (resultSet.next()) {
            ...
            String dt = resultSet.getString("WatchedOn");
            String day = dt.split(" ")[0];
            String date = dt.split(" ")[1];
            String date2 =dt.split(" ")[2];
            String time = dt.split(" ")[3];
            String totalDate = day + " " + date + " " + date2 + " " + time ;
            movie.setWatchedOn(totalDate);
    ...
    public ArrayList<Episode> getWatchedEpisodes(int profileID) throws SQLException, ClassNotFoundException {
        ...
        while (resultSet.next()) {
            ...
            String time = dt.split(" ")[3];
            String totalDate = day + " " + date + " " + date2 + " " + time ;
            episode.setWatchedOn(totalDate);
    ...
    public HashSet<Serie> getRecommendedWatchedSerie(int profileID) throws SQLException, ClassNotFoundException {
        ...
        PreparedStatement preparedStatement = databaseConnection.getConnection().prepareStatement(
                "SELECT RecommendedSerie FROM Watched_Media JOIN Profile ON Profile.Id = Watched_Media.Profile_Id JOIN Episode on Episode.Id = Watched_Media.Episode_Id JOIN Serie on Serie.id = Episode.Fk_Serie WHERE Watched_Media.Profile_Id = ? group by RecommendedSerie");
        preparedStatement.setInt(1, profileID);
        ResultSet resultSet = databaseConnection.ExecuteSelectStatement(preparedStatement);
        while (resultSet.next()) {
